...
function commonAppContracts(app, allAppIds = []) {
  ...
  add(
    ...
    // 프리캐시 '선언'을 sw.js의 './'·'index.html' 문자열로 판정하면 거의 모든 SW가 통과해 거짓 green이 된다
    // (SW 존재·문법은 service-worker 계약이 이미 실측). 앱 셸 프리캐시 완결은 표면 substring으로 확정 불가 → 정직하게 need_new_source.
    C(`c:${id}:offline-shell`, id, "pwa", "need_new_source", {},
      { severityIfFail: "info", failureClass: "availability", needNewSource: true,
        sourceNeeded: "SW 프리캐시 목록의 실측 신호(빌드 precache 매니페스트 등)",
        whyNeeded: "sw.js에 './'·'index.html'가 있다는 것만으로 앱 셸이 실제 프리캐시되는지 확정 불가(거의 모든 SW가 포함) — SW 존재·문법은 service-worker 계약이 검증",
        privacyRisk: "없음", freeImplementationOption: "배포 시 SW precache 매니페스트(파일 목록)를 노출하거나 registry에 선언",
        fallbackStatus: "표면 substring으론 확정 불가(UNAVAILABLE)",
        what: `${name} 오프라인 셸 프리캐시 — 실측 신호 필요`, userImpact: "오프라인에서 앱이 아예 열리지 않을 수 있습니다.", recommendedAction: "SW 프리캐시 목록에 앱 셸을 포함하고 확인 가능한 신호를 노출합니다." }),
...
function certbomContracts(app) {
  ...
  return [
    ...
    // '기준'·'확인'은 거의 모든 한국어 UI에 있어 LKG 배너 존재를 증명 못 함(거짓 green) → 정직하게 need_new_source.
    s("stale-banner", "user_surface", "need_new_source", {},
      { severityIfFail: "info", failureClass: "freshness", needNewSource: true,
        sourceNeeded: "LKG 기준 시각 배너 실측 신호",
        whyNeeded: "'기준'·'확인' 문자열은 거의 모든 한국어 UI에 존재해 stale 배너 표시를 증명 못 함",
        privacyRisk: "없음", freeImplementationOption: "앱이 stale 배너 표시 상태를 노출하거나 x-data-stale 헤더 사용",
        fallbackStatus: "표면 substring으론 확정 불가(UNAVAILABLE)",
        what: "LKG 기준 시각 안내 배너 — 실측 신호 필요", userImpact: "오래된 데이터를 최신처럼 보이면 안 됩니다.", recommendedAction: "기준 시각 배너를 유지하고 확인 신호를 노출합니다." }),
