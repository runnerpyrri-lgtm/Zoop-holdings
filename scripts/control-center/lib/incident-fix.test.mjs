...
test("critical/error인 self_heal 계열은 codex로 올린다(침묵 자동처리 금지)", () => {
  ...
  // severity는 자유 문자열 — info/warning이 아닌 미지의 심각도(major 등)도 조용히 자동처리하지 않고 승격한다.
  assert.equal(classifyFix({ failureClass: "availability", text: "부분 장애", severity: "major" }), "codex");
  // 경미(info) 또는 심각도 없음은 그대로 자동 재점검(회귀 방지).
  assert.equal(classifyFix({ failureClass: "freshness", text: "약간 지연", severity: "info" }), "self_heal");
  assert.equal(classifyFix({ failureClass: "observability", text: "지표 지연" }), "self_heal");
});
