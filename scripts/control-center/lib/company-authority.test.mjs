...
test("전결 분류: 시스템 상신 코드 수정은 위임 가능, 결제·계약·비밀값·홍보는 회장 전용", () => {
  ...
  for (const bad of ["신규 유료 결제 승인", "광고 캠페인 홍보 게시", "secret 키 변경", "약관 계약 갱신", "백업 없는 대량 삭제", "App Store 제출",
    ...
    // 대소문자·동의어 변형(/i + payment·stripe·app store·api key) — 예전엔 소문자·영문 결제어가 전결로 새어나갔다:
    "set up Stripe payment webhook", "rotate the api key", "refresh oauth credentials", "app store review response", "앱스토어 출시"]) {
