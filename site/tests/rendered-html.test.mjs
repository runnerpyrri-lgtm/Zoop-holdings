...
test("첫 화면은 회사 소개보다 등록된 앱 설치 선택을 먼저 제공한다", async () => {
  ...
  assert.match(html, /<title>로봄 \| 날씨·청약·러닝·자격증<\/title>/);
  ...
  assert.doesNotMatch(html, /runningcall\.vercel\.app/);
...
test("종료 앱의 문구와 링크는 모든 공개 경로에서 사라진다", async () => {
  const apps = await registryApps();
  assert.deepEqual(apps.map(({ id }) => id), ["outbom", "homebom", "runningbom", "certbom"]);

  const routes = [
    "/",
    ...apps.flatMap(({ id }) => [`/apps/${id}`, `/get/${id}`, `/privacy/${id}`]),
    "/support", "/privacy", "/terms", "/licenses",
  ];
  for (const path of routes) {
    const response = await render(path);
    assert.equal(response.status, 200, path);
    const html = await response.text();
    assert.doesNotMatch(html, /캘린더봄|노트봄|달력|음성\s*메모/, `${path}: 종료 앱 문구`);
    assert.doesNotMatch(html, /href="[^"]*(?:calendarbom|notebom)[^"]*"/i, `${path}: 종료 앱 링크`);
  }

  for (const id of ["calendarbom", "notebom"]) {
    assert.equal((await render(`/apps/${id}`)).status, 404, `apps/${id}`);
    assert.equal((await render(`/get/${id}`)).status, 404, `get/${id}`);
  }
});
...
test("Pages 프리렌더에는 앱 소개·설치·정책이 모두 포함된다", async () => {
