  ...
  return (
    <PageShell current="apps">
      <section className={`product-page ${app.tone}`}>
        ...
        <div className="product-hero"><div className="product-copy"><p className="card-eyebrow">{app.eyebrow}</p><h1>{app.heroTitle}</h1><p>{app.heroBody}</p><div className="product-actions"><Link className="button primary" href={app.installPath}>{app.accessLabel} <span aria-hidden="true">→</span></Link><a className="button secondary" href={app.webUrl} target="_blank" rel="noopener noreferrer">웹으로 먼저 체험 <span aria-hidden="true">↗</span></a></div><a className="product-contact" href={contactHref(app.name)}>문의·의견 보내기</a></div><div className="product-score" aria-label={`${app.name} 버전`}><small>현재 버전</small><strong>v{app.version}</strong><span>{app.mobileStatus.toUpperCase()}</span></div></div>
