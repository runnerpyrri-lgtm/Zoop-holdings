...
function loadContracts(){ // 1회 로드 후 도착 시 한 번만 재렌더(무한 재렌더 금지)
  if(CONTRACTS||CONTRACTS_LOADING)return;
  CONTRACTS_LOADING=true;
  // 공개 미리보기: 계약 '정의'만 번들(실시간 판정 결과는 맥에서). 실앱은 /api/health-contracts.
  fetchJson(preview?"./contracts.json":"/api/health-contracts").then(v=>{CONTRACTS=v;if(["automation","company"].includes(CURRENT))renderScreen();}).catch(()=>{CONTRACTS={ok:false};});
}
...
function loadOrg(){ // 1회만 불러오고, 도착했을 때 회사 화면이면 한 번만 다시 그린다(무한 재렌더 금지)
  if(ORG||ORG_LOADING)return;
  ORG_LOADING=true;
  // 공개 미리보기: 조직 정본을 번들 파일에서(정적). 실앱은 /api/organization.
  fetchJson(preview?"./org.json":"/api/organization").then(o=>{ORG=o;if(CURRENT==="company")renderScreen();}).catch(()=>{ORG={ok:false};});
}
