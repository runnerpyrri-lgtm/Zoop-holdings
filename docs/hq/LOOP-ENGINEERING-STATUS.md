...
- **8. 성장(유지보수 넘어)** — 성장 제안 파이프라인(nextActions→성장 Loop→전결→Codex) 존재. **새 기능·디자인의 창의적 생성은 Codex(맥) 실행 단계**에서 이뤄지므로, 맥 연결 후 성장 로드맵을 Codex에 태워 실제 신기능을 만든다(정직: 현재 HQ 결정론 엔진은 아이디어를 '실행'까지는 맥 필요).
