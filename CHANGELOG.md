...
- 모바일 첫 화면을 실행 우선 앱 홈으로 바꾸고, 세 앱을 96px 높이의 풀폭 실행 카드로 배치했다.
- 모바일에서는 긴 브랜드 설명을 첫 화면에서 덜어내고, 데스크톱의 소개형 히어로 구성은 유지했다.
- 고정된 시각과 `LIVE` 표현을 알림 예시로 바꿔 실제 실시간 상태로 오인되지 않게 했다.
- 빠른 실행 안내 대비와 보조 링크 터치 영역을 보완하고, 360·390·414px 모바일과 데스크톱 화면을 다시 검증했다.
