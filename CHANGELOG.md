...
### 추가 (robom 브랜드 사이트)
- 회사 이름을 `robom`으로 정리하고, 따뜻하고 화사한 브랜드 홈페이지를 `site/`에 추가했다.
- 한쪽 안경을 쓴 로봄 캐릭터와 `로봄 robom` 워드마크, 세 앱 포트폴리오, 운영 원칙을 한 화면에 구성했다.
- 모바일 반응형·키보드 포커스·모션 감소 설정을 포함하고 GitHub Pages 배포 워크플로를 추가했다.
