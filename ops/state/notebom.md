...
- 패밀리 규격: 1.0.0.
- 핵심 기능: 한 번 눌러 녹음, 5초 단위 원음 청크 저장, 중단 녹음 복구, 저장 직후 무료 기기 내 Whisper 자동 텍스트 변환, 원문·사용자 수정본 분리, 재인식, 템플릿, 검색·필터, AI 전달 패킷, 캘린더봄/ICS 전달, 전체 원음 ZIP 백업·merge/replace 복원.
- 개인정보 기본값: 로그인·서버 동기화·광고·분석 OFF, 원음과 텍스트는 브라우저 IndexedDB에 저장, 기본 음성 변환은 자체 배포 모델로 기기 안에서 처리한다. 사용자가 별도로 브라우저 인식을 선택할 때만 외부 처리 가능성을 고지한다.
...
- [x] 저장된 원음을 계정·유료 API 없이 자동 텍스트로 바꾸고 실패 시 원음에서 재시도할 수 있게 한다.
...
- 브라우저의 지속 백그라운드 녹음은 지원을 과장하지 않는다. 기기 내 STT의 속도·정확도·메모리 한계와 실기기 미검증 범위를 명시한다.
