...
macOS 로그인 뒤에도 본부를 자동으로 켜고, 서버가 종료되면 다시 실행하려면 아래 설치 파일을 한 번 실행한다.

```bash
bin/install-robom-hq-autostart.command
```

자동 시작 실행본은 macOS 백그라운드 접근 제한을 피하기 위해 `~/Library/Application Support/RobomHQ`에 설치된다. 회사 기록은 그 안에 따로 보존되며 설치 파일을 다시 실행해도 기존 기록을 덮어쓰지 않는다.

오피스의 `전원 업무 시연`은 모든 직원이 일하는 모습을 확인하는 화면 연출이다. 실제 업무 기록과 섞이지 않으며, 실제 작업이 들어오면 해당 직원은 실시간 업무 상태를 우선 표시한다.
