...
## 2026-07-11 · 회사 브랜드 변경

### D21. 회사 이름은 robom, 브랜드 사이트는 관제 저장소에서 운영
- 결정: 회사 이름을 `robom`으로 바꾸고, `로봄 robom` 워드마크와 한쪽 안경을 쓴 로봇 캐릭터를 공식 브랜드로 사용한다.
- 브랜드 사이트는 제품 앱과 분리해 관제 저장소의 `site/` 정적 자산으로 관리한다.
- 앱 원본 저장소와 기존 배포 주소는 바꾸지 않으며, 브랜드 사이트만 별도 GitHub Pages 워크플로로 배포한다.
